...
def load_student_checkpoint(student: torch.nn.Module, ckpt_dir: str, logger: logging.Logger, is_main: bool):
    state_path = os.path.join(ckpt_dir, "pytorch_model.bin")
    if not os.path.exists(state_path):
        raise FileNotFoundError(f"未找到 checkpoint 权重文件: {state_path}")
    state = torch.load(state_path, map_location="cpu")
    missing, unexpected = student.load_state_dict(state, strict=False)
    if is_main:
        logger.info("已从 %s 加载学生权重。missing=%d unexpected=%d", ckpt_dir, len(missing), len(unexpected))
        if missing:
            logger.debug("[checkpoint] missing keys: %s", missing[:10])
        if unexpected:
            logger.debug("[checkpoint] unexpected keys: %s", unexpected[:10])
...
def main():
    ...
    if init_ckpt:
        ...
        if is_main:
            logger.info(f"从 checkpoint {init_ckpt} 初始化学生权重（假定已包含混合结构）。")
        load_student_checkpoint(student, init_ckpt, logger, is_main)
    else:
    ...
    if layer_indices and not init_ckpt:
        if is_main:
